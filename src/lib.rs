...
//!
//! `schema/signal.schema` is the source of truth for the Spirit wire contract.
//! Shared domain taxonomy types are consumed from `signal-domain` and re-exported
//! through this crate for compatibility. The checked-in signal file under
//! `src/schema/` is generated by `schema-rust`. Runtime actors, sockets,
//! storage, guardian calls, and daemon lowering live in `spirit`.
...
pub const DOMAIN_RUST_SOURCE: &str = signal_domain::DOMAIN_RUST_SOURCE;
...
impl Input {
    pub fn validate(&self) -> Result<(), ValidationError> {
        match self {
            ...
            Self::Intent(intent) => intent.payload().validate(),
            Self::TextSearch(search) => search.payload().validate(),
            Self::Lookup(_)
...
impl OperationKind {
    pub fn from_input(input: &Input) -> Self {
        match input {
            ...
            Input::Intent(_) => Self::Intent,
            Input::TextSearch(_) => Self::TextSearch,
...
impl DomainMatch {
    ...
    pub fn matches(&self, entry_domains: &Domains) -> bool {
        match self {
            Self::Any => true,
            Self::Partial(partial) => partial.payload().matches_spirit_domains(entry_domains),
...
impl Domains {
    pub fn from_strings(labels: Vec<String>) -> Self {
        ...
        for label in labels {
            Self::push_from_label(&label, &mut domains);
        }
        Self::new(domains)
    }
    ...
    fn push_from_label(label: &str, domains: &mut Vec<Domain>) {
...
pub trait SpiritDomainScopes {
    fn validate(&self) -> Result<(), ValidationError>;
    fn is_empty(&self) -> bool;
    fn iter(&self) -> impl Iterator<Item = &DomainScope>;
    fn matches_spirit_domains(&self, domains: &Domains) -> bool;
}

impl SpiritDomainScopes for DomainScopes {
    fn validate(&self) -> Result<(), ValidationError> {
        if self.is_empty() {
    ...
    fn matches_spirit_domains(&self, domains: &Domains) -> bool {
        self.matches_any_domain(domains.payload())
    }
}
