...
| Authored schema uses strict positional dotted syntax. | `schema/signal.schema` applies generics as positional dotted forms (`Vector.Domain`, `Optional.Antecedent`). Its remaining named-brace `Stream` application (`IntentEventStream (Stream { … })`, line 174) and single-colon import paths (`signal-domain:domain:Domain`) are non-conforming: generic application is positional dotted and parameter names live only in the definition, so these must be rewritten to positional dotted application and dotted import paths (`signal-domain.domain.Domain`). |
