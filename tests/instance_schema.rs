...
use nota_next::{
    InstanceSchema, InstanceSchemaBody, NotaDecodeTraced, NotaSource, TypeReference,
};
use schema_next::{InstanceSchemaText, SourceReference};
use signal_spirit::{
    Certainty, Domain, DomainMatch, Entry, Input, Kind, Magnitude,
};
...
fn bare_optional_leaf_variant_traces_an_empty_optional() {
    ...
    assert!(matches!(optional.body(), InstanceSchemaBody::Optional(None)));
}
